window.OBSOverlayUserEditor = (() => {
  ...
  function createUserRow(initialValues, deps) {
    ...
    card.addEventListener("paste", async (event) => {
      ...
      try {
        await deps.applyUserImageFile(user.internalId, dataUrlField, file);
        deps.setStatus("クリップボードの画像を貼り付けました。");
      } catch (error) {
        deps.setStatus("クリップボード画像の貼り付けに失敗しました。", "error", true);
      }
    });
  ...
  function renderActiveUserEditor(deps) {
    ...
    if (moveRightButton) {
      moveRightButton.disabled = activeIndex === -1 || activeIndex >= deps.getUsersState().length - 1;
    }
